...
const PostCellMedium = ({
  ...
  return (
    <div
      className={`post-cell post-cell-medium ${
        index === 1 && "post-cell-first"
      }`}
    >
        ...
        <div className="post-subheading">
          ...
          {showDate && date && (
            <div className="post-date">
              <Link to={`/date/${date.replace(new RegExp("/", "g"), "-")}`}>
                {date}
              </Link>
            </div>
          )}
        ...
        {showActions && (
          <div className="post-actions">
            ...
            {showFinishCheckout && hasPendingCheckout(post) && (
              <LinkContainer
                className="post-finish-checkout"
                to={`/sponsor/checkout/${_id}`}
              >
      ...
      {showStatus &&
        [postStatus.published, postStatus.scheduled].includes(post.status) && (
